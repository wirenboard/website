...
1. Добавьте обложку в папку [public/img/articles](/public/img/articles) в формате `jpg`, `webp`, `png`, разрешение по горизонтали 500px.
...
В отличие от добавления ссылки, здесь отсутствует поле url (он будет формироваться из именования md файла) и есть контент самой статьи:
...
Если в статье есть картинки, то их стоит разместить в папке `public/img/articles/ARTICLE_NAME`. Формат картинок может быть `jpg`, `webp`, `png`. Размер изображений не более 3 Мб. Разрешение картинки любое, рекомендуемое разрешение по горизонтали 1200px и более.

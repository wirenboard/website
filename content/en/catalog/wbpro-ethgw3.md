...
The WBPRO-ETHGW3 gateway is designed to connect devices with RS‑485 and WBIO (I/O modules) interfaces over Ethernet and Wi‑Fi.
...
If the device you use to connect to the gateway supports mDNS, the module is reachable as **wbpro-ethgw3-XXXXXX.local** on any network interface.
...
In the basic configuration, the module is powered by 9-28 V DC and a PoE module, which supports the following standards:
...
If WBPRO-ETHGW3 is connected to one of the network interfaces of a Wiren Board controller that is linked to Wiren Board Cloud, you can proxy the module’s web UI through the cloud and access it remotely. See the documentation for details.
...
WBPRO-ETHGW3 supports Fast Modbus in all modes: **transparent gateway** and **Modbus TCP**.
...
WBPRO-ETHGW3 is based on an **ESP32** microcontroller and ships with our stock firmware that implements gateway functionality.

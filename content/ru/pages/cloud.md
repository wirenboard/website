...
Открывайте веб-интерфейсы сервисов на контроллере — Home Assistant, Node-RED, Zigbee2MQTT и собственные приложения — по прямой ссылке, без проброса портов и VPN. Каждый сервис получает отдельный адрес вида `https://XXXXXXXX-PPPP.apps.wirenboard.cloud`, доступный пользователям организации с любого устройства.  

На одном контроллере можно открыть до 20 веб-сервисов. Прокидываются только сервисы по HTTP/HTTPS — для MQTT, Modbus TCP и других протоколов функция не работает. Под сервисы нельзя занять системные порты: 22 (SSH), 80 (встроенный веб-интерфейс), 1883, 8883, 18883–18886 (MQTT) и 6720 (knxd). Остальные можно использовать без ограничений.

:gallery{
    :data='[
        ["cloud/ru-services-buttons.png", "Кнопки быстрого доступа к сервисам"],
        ["cloud/ru-services-add.png", "Добавление сервиса"]
    ]'
}

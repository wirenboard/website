---
article: "CTSA024"
name: "CTSA024"
title: "Трансформатор тока"
type: "Трансформатор тока до 290 А, 24 мм,"
sub_title: "На ток до 290 А, 24 мм диаметр провода"
cover: "ctsa024/ctsa024_front.jpeg"
catalogCover: "ctsa024/ctsa024_front.jpeg"
documentation: "https://wiki.wirenboard.com/wiki/CTSA024"
meta: "Один разъёмный трансформатор тока для использования с измерителями серии WB-MAP."
keywords: "WB-MAP, CTSA024, тт, трансформатор тока"
images: [
  ["ctsa024/ctsa024_front.jpeg"]
]
---
::product
#description
Один разъёмный трансформатор тока для использования с измерителями серии WB-MAP.

Измеряемый ток от 0.35 до 290 А.

#info
::product-section{title="Технические характеристики"}
:photo{
  src="ctsa024/ctsa024_current.png"
  width="500px"
  float="right"
}
- измеряемый ток от 0.35 до 290 А;
- номинальный первичный ток 35 А;
- класс точности 0.5S;
- коэффициент трансформации 1:5000 (точное значение на этикетке);
- диаметр кабеля до 24 мм;
- тип сердечника — разъёмный;
- материал сердечника — феррит;
- вес (без проводов) 190 г;
- длина провода 2 м.
::

:include{
  path="/catalog/includes/current_transformers_list"
}

::

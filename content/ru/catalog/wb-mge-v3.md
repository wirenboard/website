...
Питание модуля через специальные клеммы подаётся на подключённые к портам RS-485 устройства. Есть защита по току 2.5 A на оба порта, а также программное отключение и сохранение состояния при перезагрузке. При работе от PoE на выход подаётся 24 В постоянного тока, а допустимый ток на Vout 500 мА.
